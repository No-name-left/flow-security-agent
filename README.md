...
The unique long-term code branch is `main`; the current synchronized main baseline is `607c40bf3c494411503be6e960d3b88450332e22`, which contains the tagged pre-model integration baseline `3ab33e36c8508bcd31afac2e12c094ae1fe0a964`. The Edge split/SFT revision is isolated on `fix/edge-paper-split-revision` until its authorized commit/merge. After that freeze, the next engineering task is the narrow, whitelist-based Production-to-Runtime adapter; real provider/model setup, Qwen download and training remain separately authorized future work.
