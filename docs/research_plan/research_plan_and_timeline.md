...
> Derived from the canonical `research_plan_detailed.md`; updated 2026-08-11.
>
...
| Application / Sanitized Payload / Production RAG | UNAVAILABLE engineering state; required on-demand final capabilities |
...
| SFT / RL / Unknown / formal benchmark | NOT RUN / NOT RUN / NOT FROZEN / NOT RUN |
...
The Fine Head is the sole trained fine-class source; coarse uses deterministic mapping. GRPO optimizes Evidence behavior, not Fine Head correctness as a group-relative reward. Fine classification remains a separate CE term.
...
| B | Transformers/PEFT harness; pooling; LoRA inventory assertion; serialization v1; Prompt/schema v1; Application/Payload contracts; RAG Evidence Contract | **NEXT IMPLEMENTATION PHASE** |
| C | Raw Near Qwen and LightGBM/XGBoost/RF strong baselines | NOT STARTED |
| D | Build bounded multi-stage Near SFT corpus | legal K_known TRAIN only |
| E | DeepSeek Flash Teacher + deterministic rules/masking + consistency filter + bounded human audit | NOT STARTED |
...
This documentation task freezes the execution protocol only. `SFT_RUN=false`, `RL_RUN=false`, `UNKNOWN_ALGORITHM_FROZEN=false`.

**NEXT IMPLEMENTATION PHASE: Phase B — Training Protocol Readiness.** Do not start baseline, SFT, DeepSeek batch generation, GRPO, Unknown, U_final or Agent experiments without a new authorized task.
