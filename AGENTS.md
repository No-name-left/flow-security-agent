...
- The remote server bootstrap, official dataset recovery and Production Data Freeze are complete. `main` is the unique long-term branch and contains the pre-model baseline `3ab33e36c8508bcd31afac2e12c094ae1fe0a964`, tagged `baseline-pre-model-20260811`, with Production, the Agent Runtime foundation and provider-neutral real-backend preparation. The next boundary task is a small explicit Production-to-Runtime adapter and its leakage tests. Keep data/artifact/model roots outside the Git checkout and configurable through environment/configuration rather than hard-coded local Windows or server paths.
