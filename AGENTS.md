...
- The remote server bootstrap, official dataset recovery and Production Data Freeze are complete. `main` is the unique long-term branch; its synchronized baseline is `607c40bf3c494411503be6e960d3b88450332e22` and it contains the tagged pre-model integration baseline `3ab33e36c8508bcd31afac2e12c094ae1fe0a964`, with Production, the Agent Runtime foundation and provider-neutral real-backend preparation. The Edge split/SFT revision remains isolated on `fix/edge-paper-split-revision` until integrated. After that freeze, the next boundary task is a small explicit Production-to-Runtime adapter and its leakage tests. Keep data/artifact/model roots outside the Git checkout and configurable through environment/configuration rather than hard-coded local Windows or server paths.
