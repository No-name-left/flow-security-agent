...
- The remote server bootstrap, official dataset recovery and Production Data Freeze are complete. The frozen Production branch has been integrated with the Agent Runtime foundation and provider-neutral real-backend preparation on a dedicated integration branch; the next boundary task is a small explicit Production-to-Runtime adapter and its leakage tests. Keep data/artifact/model roots outside the Git checkout and configurable through environment/configuration rather than hard-coded local Windows or server paths.
